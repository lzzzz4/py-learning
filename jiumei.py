...
# 九妹爬虫
soup, lis, series_count = goToHomeSite("腿", 1)
...
# 当页数不为1的时候,获得总的页数，随机访问一页
    ...
    soup, lis, series_count = goToHomeSite("腿", num_page)
...
for i in range(1, pictotalAmout + 1):
    ...
    if (imgresponse.status_code == 200):
        ...
        if not os.path.exists(path):
            os.mkdir(path)
        finpath = path + "/" + str(uuid.uuid1())[0:10] + final_pic_src[final_pic_src.rfind("/") + 1:final_pic_src.rfind(
            ".jpg")] + ".jpg"
        print(finpath)
